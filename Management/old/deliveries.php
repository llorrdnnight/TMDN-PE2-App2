<?php
    ...
    if (!isset($_SESSION)) { session_start(); }; // Check if the session is set else redirect to login page
    if (!isset($_SESSION['employee_id']))
        header("Location: login.php");

    $deliveries = json_decode(file_get_contents("http://10.128.30.7:8080/api/parcels"), true);
    $flights = json_decode(file_get_contents("http://10.128.30.7:8080/api/flights"), true);
    ...
    function getFilteredArray($arr)
    {
        ...
        $filterList = array("orderId", "flightId", "parcelTypeId", "priority", "departureTimeStamp", "arrivalTimeStamp", "isAllocated");
